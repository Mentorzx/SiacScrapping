...
class MainWindow(GenericWindow):
    ...
    def __init__(self, notion_factories: dict[str, any]) -> None:
        """
        ...
        self.notion_factories = notion_factories
    ...
    def _login(self) -> None:
        ...
        """
        notion_data = self._gather_login_data()
        factory = NotionRequestFactory(config, config["notion_login"]["main_db_id"])
        response = factory.check_connection()
        if response[0] == 504:
            messagebox.showerror(
                "TimeoutError",
                "Unable to connect to Notion. Retry later or check you connection and contact Notion Support.",
            )
            return
        if response[0] != 200:
            messagebox.showerror(
                json.loads(response[1])["code"].capitalize(),
                json.loads(response[1])["message"],
            )
            return
