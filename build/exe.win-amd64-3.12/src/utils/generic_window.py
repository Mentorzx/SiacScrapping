...
class GenericWindow:
    ...
    def __init__(
        ...
        button_texts: dict[str, list[str]] = None,
        button_actions: dict[str, list[Callable[[], None]]] = None,
        resizable: bool = True,
    ) -> None:
        """
    ...
    def _setup_tabs(self, tab_titles: list[str]) -> None:
        ...
        for idx, title in enumerate(tab_titles):
            ...
            self.tab_view.tab(title).grid_rowconfigure(0, weight=1)
            self.tab_view.tab(title).grid_columnconfigure(0, weight=1)
